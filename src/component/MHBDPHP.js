...
/*        
    ...
    this.objIzm={}
    this.setIzmenKey=function(key, array, fun_creat, fun_setParam, fun_saveFile){ 
        
        let oo={}
        oo.key=key;
        oo.array=array;
        oo.creat=fun_creat;
        oo.setParam=fun_setParam;
        oo.saveFile=fun_saveFile
        if(this.objectBase[key]){
            this.objectBase[key]=array;
    ...
    this.save=function(){ 
        ...
        if(this.boolNotPHP==true)return
        $.post(
    ...
    this.creat=function(key,fun){
        ...
        if(this.objIzm && this.objIzm[key]){
            
            this.objIzm[key].creat(fun)
    ...
    this.getIdResObj=function(keyType, id, bool){
        if(keyType==undefined || id==undefined)return null;

        
        for (var i = 0; i < self.objectBase[keyType].length; i++) { 

            if(self.objectBase[keyType][i].files!==undefined){
                for (var j = 0; j < self.objectBase[keyType][i].files.length; j++) {  
            ...
            if(self.objectBase[keyType][i].icon!==undefined && self.objectBase[keyType][i].icon.url!==undefined){
                return self.objectBase[keyType][i].icon.url;
            }
    ...
    this.saveFile=function(file,keyType,id,fun){       
        ...
        if(self.objIzm && self.objIzm[keyType]){           
            self.objIzm[keyType].saveFile(file,id,fun)
            //if(oo[param]){                
                //self.objIzm[keyType].setParam(id,param,value,fun)
    ...
    this.clearFile=function(keyType,id,fun){
        ...
        if(oo2){
            ...
            if(oo2.fname==undefined)fun(null);
            if(oo2.fname[ii]==undefined)fun(null);
            var n=oo2.fname[ii]
    ...
    this.getKeyId=function(key,id,fun, bool){  
        ...
        if(self.objDin[key]==undefined)this.objDin[key]={}
        if(self.objDin[key][id]!==undefined){
            if(fun)fun(self.objDin[key][id])
            return self.objDin[key][id]
        }   


        for (var i = 0; i < self.objectBase[key].length; i++) {
            if(self.objectBase[key][i].id==id){
                self.objDin[key][id]=self.objectBase[key][i]
                if(self.objDin[key][id].tN==undefined)self.objDin[key][id].tN = key;
                if(self.objDin[key][id].uuid==undefined)self.objDin[key][id].uuid = self.generateRendom();
    ...
    this.setParam=function(keyType,id,param,value,fun){        
        ...
        if(self.objIzm && self.objIzm[keyType]){
            ...
            if(oo[param]!=null && oo[param]!=undefined){
